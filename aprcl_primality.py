...
class APRCLPrimality:
    ...
    def e(t):
        ...
        for q in range(2, t+2):
            if t % (q-1) == 0 and APRCLPrimality.isprime_slow(q):
                s *= q ** (1+APRCLPrimality.v(q,t))
                q_list.append(q)
        return 2*s, q_list
    ...
    @staticmethod
    def calc_f(q):
        g = APRCLPrimality.smallest_primitive_root(q)
        m = {}
    ...
    @staticmethod
    def calc_J(p, k, q):
        return APRCLPrimality.calc_J_ab(p, k, q, 1, 1)

                
    # calculate J_3(q)（p=2 and k>=3）
    @staticmethod
    def calc_J3(p, k, q):
        j2q = APRCLPrimality.calc_J(p, k, q)
        j21 = APRCLPrimality.calc_J_ab(p, k, q, 2, 1)
        j_ret = j2q * j21
        return j_ret
    ...
    @staticmethod
    def calc_J2(p, k, q):
        j31 = APRCLPrimality.calc_J_ab(2, 3, q, 3, 1)
    ...
    def APRtest_step4b(p, k, q, N):
        J = APRCLPrimality.calc_J3(p, k, q)
        # initialize s1=1
        s1 = JacobiSum(p,k,q).one()
        # J3^Theta
        for x in range(p**k):
        ...
        if N%8 in [1,3]:
            ...
            J2_delta = APRCLPrimality.calc_J2(p,k,q)
    ...
    def APRtest_step4(p, k, q, N):
        ...
        if p>=3:
            result, l_p = APRCLPrimality.APRtest_step4a(p, k, q, N)
        elif p==2 and k>=3:
            result, l_p = APRCLPrimality.APRtest_step4b(p, k, q, N)
        elif p==2 and k==2:
            result, l_p = APRCLPrimality.APRtest_step4c(p, k, q, N)
        elif p==2 and k==1:
            result, l_p = APRCLPrimality.APRtest_step4d(p, k, q, N)
        else:
            print("Error in APRtest_step4")
    ...
    def isPrime(N):
        ...
        # Step 2
        l = {}
        fac_t = APRCLPrimality.prime_factorize(t)
        for p, k in fac_t:
        ...
        for q in q_list:
            if q == 2:
                continue
            fac = APRCLPrimality.prime_factorize(q-1)
            for p,k in fac:

                # Step 4
                result, l_p = APRCLPrimality.APRtest_step4(p, k, q, N)
        ...
        for p, value in l.items():
            if value==0:
                ...
                while count < 30:
                    q = p*i+1
                    if N%q != 0 and APRCLPrimality.isprime_slow(q) and (q not in q_list):
                        count += 1

                        k = APRCLPrimality.v(p, q-1)
                        # Step 4
                        result, l_p = APRCLPrimality.APRtest_step4(p, k, q, N)
